...
import { hardTgmConfig, normalTgmConfig } from "./TetrisService";
...
export class TitlePlay implements IPlay {
    ...
    stepFrame(gl: WebGL2RenderingContext, stick: StickData): IPlay {
        if (stick.isStart(true)) {
            return new GamePlay(gl, this.level === 0 ? normalTgmConfig : hardTgmConfig);
        }
        ...
        this.fontRender.drawFrame(gl, [-0.55 + this.level * 0.6, 0.15, 0.5, 0.2], [0, 0, 0, 0], [1, 1, 0.5]);
        ...
        this.fontRender.draw(gl, "PRESS " + stick.getButtonName(ButtonType.Start), [-0.4, 0.5, 0.8, 0.1], [1, 1, 1]);
